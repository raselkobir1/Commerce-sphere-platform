...
export class DashboardPage implements OnInit {
  ...
  ngOnInit(): void {
    this.api.get<Paged<Product>>('/api/products', { pageNumber: 1, pageSize: 200 }).subscribe((r) => {
      this.products.set(r.items);
      this.totalProducts.set(r.totalRecords || r.items.length);
    });
    this.api.get<Paged<User>>('/api/auth/users', { pageNumber: 1, pageSize: 1 }).subscribe((r) => this.customers.set(r.totalRecords));
    this.api.get<Paged<InventoryItem>>('/api/inventory', { pageNumber: 1, pageSize: 200 })
      .subscribe((r) => this.lowStock.set(r.items.filter((i) => i.quantityAvailable <= i.reorderLevel)));
  }
}
