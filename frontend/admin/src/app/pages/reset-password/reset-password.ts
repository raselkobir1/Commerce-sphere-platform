      ...
      <div class="login-main">
        @if (!token()) {
          <div class="login-card" style="text-align:center">
            <div class="lead">Invalid link</div>
            <p class="sub">This password reset link is missing or malformed. Request a new one.</p>
            <a class="btn btn-primary" style="width:100%; justify-content:center" routerLink="/forgot-password">Request a new link</a>
          </div>
        } @else if (done()) {
          <div class="login-card" style="text-align:center">
            <div class="lead">Password reset</div>
            <p class="sub">Your password has been updated. You can now sign in.</p>
            <a class="btn btn-primary" style="width:100%; justify-content:center" routerLink="/login">Sign in</a>
          </div>
        } @else {
          <form class="login-card" (ngSubmit)="submit()">
            <div class="lead">Reset your password</div>
...
export class ResetPasswordPage implements OnInit {
  ...
  submit(): void {
    ...
    this.api
      .post('/api/auth/password/reset', { token: this.token(), newPassword: this.newPassword }, { toastSuccess: false })
      .subscribe({
