...
@Component({
  ...
  imports: [RouterLink, BdtPipe, TranslatePipe],
  template: `
    <div class="card">
      <a class="thumb" [routerLink]="['/product', product().id]"
         [style.background-image]="product().imageUrl ? 'url(' + product().imageUrl + ')' : null">
        @if (product().stock === 0) { <span class="tag out">{{ 'product.outOfStock' | t }}</span> }
        @else if (product().stock <= 10) { <span class="tag low">{{ i18n.t('product.onlyLeft', { n: product().stock }) }}</span> }
